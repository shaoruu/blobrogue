...
routes accept only a validated `worldId` and, for warp, a positive integer `floor` plus optional
per-player `loadouts`. Any extra field is rejected before dispatch. Warp floors are bounded to
`1..1000`.
...
| `POST /v1/worlds/warp` | `{ worldId, floor, loadouts? }` | no | move the named live co-op room to a freshly generated floor, optionally replacing selected players' builds |
    ...
    "loadouts":[
      {
        ...
        "kitId":"phantom",
        "hp":2.5,
        "weapons":["pistol","shotgun","tesla","nailer","margin_call","red_pen"],
        "blessings":[
...
player advances together. Warp keeps each player's kit, inventory, blessings, and health unless a
matching loadout is supplied; downed players receive the normal descent rescue before the fresh
floor is generated. A loadout's `player` selector matches the exact display name shown by
`GET /v1/worlds`, verified account identity, or authoritative player id. Ambiguous or missing
players are skipped and reported. A supplied weapon list replaces that player's hotbar through the
normal capped acquire/equip path, and the last accepted weapon is equipped. A supplied blessing
list replaces the blessing build and applies each requested level through the normal pickup path.
Unknown weapon, blessing, or kit ids are skipped, logged by the game server, and returned in the
response without aborting the room-wide warp. `hp` is applied after kit and blessings and is
clamped to the resulting maximum. Force-open removes
