...
| `POST /v1/worlds/snapshot` | `{ worldId }` | no | atomically persist the room's floor, builds, and reconnect seats |
| `POST /v1/worlds/restore` | `{ worldId }` | no | restore a saved room onto a fresh floor generated by the current build |
...
WebSocket messages, and direct unsigned HTTP requests cannot invoke these actions. PVP rooms are
rejected.
...
curl -X POST -H "Authorization: Bearer $BRC_ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"worldId":"room:ABCD:g1"}' \
  http://127.0.0.1:8091/v1/worlds/snapshot

curl -X POST -H "Authorization: Bearer $BRC_ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"worldId":"room:ABCD:g1"}' \
  http://127.0.0.1:8091/v1/worlds/restore
```
...
Run snapshots use **build+floor fidelity**. The file stores the authoritative player projection
(kit, weapons and weapon state, blessings/mods, HP, coins, ult/pet state), offer history, run seed,
floor, party-size lock, weapon bag, and each verified identity's current reconnect credential. It
does not replay enemy positions or a partially completed encounter. Restore generates floor N
from scratch with the currently deployed code, places every saved co-op player at the same fresh
spawn, restores their builds and HP, and installs all reconnect seats as one operation. Downed
players return standing with at least 1 HP. Restored runs are ineligible for account progression
receipts to prevent replaying one snapshot for duplicate rewards.

Snapshots land at `/var/lib/blobrogue/run-snapshots/<worldId>.json` by default
(`GS_RUN_SNAPSHOT_DIR` overrides it), mode 0600. Once a snapshot exists, deploy FLUSH refreshes it,
keeps the generation active, and closes clients with a reconnectable service-restart code. The new
game-server process restores saved rooms before accepting WebSocket resumes. The explicit restore
call is idempotent after this boot restore, so it is safe to run as the post-deploy confirmation.
The file is removed when the restored room naturally empties.
