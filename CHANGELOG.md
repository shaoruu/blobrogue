...
- **The F30 finale keeps you guessing:** the Hollow Choir now re-tunes its hall on every phase — resonant pillars crumble and a fresh gapped ring rises (always with a clear route through), so the fight reads differently each phase — and its verses no longer summon only ghosts: each gathering verse is a different readable spectral kin (the drifting revenant, the hollow dead), so no two silence-the-choir loops feel the same. The earned-window core is untouched: you still open its window by silencing the verse.
