...
import {
  ...
  DIFFICULTIES, DEFAULT_DIFFICULTY, difficultyActiveCap, difficultyBossAddCap,
...
function releaseReinforcements(w: WorldState, dt: number, ev: SimEvent[]): void {
  ...
  w.enemies.push(next);
  // Wave pacing = authored stagger ÷ biome rate × the mode's reinforcement-interval knob
  // (studio gate §1: casual 1.25× slower waves, brutal 0.85× faster).
  w.spawnReleaseCd = (REINFORCE_STAGGER / BIOME_PRESSURE[biomeIndexForFloor(w.floor)].reinforceRate) * DIFFICULTIES[w.difficulty].reinforceIntervalMult;
...
function updateBoss(w: WorldState, e: Enemy, dt: number, ev: SimEvent[]): void {
  ...
  // Interval and live cap carry the mode's boss-add knobs (studio gate §1: casual 1.20×
  // interval / −1 cap, brutal 0.85× / +1) — HP and phase mechanics stay authored.
  if (!boss.roar) {
    boss.addTimer -= dt;
    if (boss.addTimer <= 0) {
...
function bossBeginAttack(w: WorldState, e: Enemy, ev: SimEvent[]): void {
  const boss = e.boss!;
  boss.attackCount++;
  // The idle cooldown between committed attacks carries the mode pacing knob (§1 "enemy/
  // boss idle attack CD"); windups, locks and recoveries stay authored in every mode.
...
function spawnEnemyBullet(w: WorldState, x: number, y: number, angle: number, speed: number, radius: number, damage: number, color: string, life: number): void {
  // The ONE seam every enemy projectile passes through, so the mode's projectile-speed
  // knob (§1: casual 0.90× / brutal 1.10×) applies uniformly — dodge windows widen or
  // tighten while damage and telegraphs stay authored. Player bullets are never scaled.
  const v = speed * DIFFICULTIES[w.difficulty].projectileSpeedMult;
  w.bullets.push({
    x, y,
