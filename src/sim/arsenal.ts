...
export const ARSENAL: Record<WeaponId, WeaponManifestEntry> = {
  ...
  backtalk: {
    ...
    novelty: {
      nearest: "ricochet",
      axes: ["timing", "priority"],
      note: "The Ricochet bounces YOUR round off walls; Backtalk opens a frontal PARRY window that catches an ENEMY shot and returns it at 1.15x, rewarding a read of the threat, not geometry.",
    },
    idealRange: "mid", target: "single",
    strength: "A well-timed catch answers a heavy shot with a bigger one on the enemy's own budget.",
    weakness: "An empty window is a feeble stub on a real cooldown, and boss hazards never catch.",
    resource: "none", status: "none",
