...
import { createWorld, stepPlayerPhase, loadFloorIntoWorld, resolveWarmthDrain } from "../sim/world.js";
  ...
  advance(dt: number): void {
    ...
    while (this.accumulator >= FIXED_DT && steps < MAX_STEPS_PER_FRAME) {
      ...
      if (this.isReady() && this.isSnapSeenOnSocket) {
        ...
        this.sendMsg({ t: "input", seq, mx: cmd.moveX, my: cmd.moveY, aim: cmd.aim, fire: cmd.firing, dash: cmd.dash, act: cmd.interact === true, ult: cmd.ult === true, pulse: cmd.pulse === true, pet: cmd.petAbility === true, ak: cmd.arenaUltKit ?? "", ackEv: this.lastEventId, ackSnap: Math.max(0, this.lastSnapSseq) });
  ...
  remotePlayers(): RemotePlayer[] {
    ...
    return snap.players.map((p) => {
      ...
      return {
        ...
        spawnGraceT: p.sgr,
        spawnShieldT: p.ssh,
        spawnProtectionStartedTick: p.spo,
        spawnHardGraceEndsAtTick: p.sge,
        spawnShieldEndsAtTick: p.sse,
        authoritativeTick: snap.tick,
