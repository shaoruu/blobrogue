...
export const CHANGELOG: ChangelogSection[] = [
  {
    ...
    date: "unreleased",
    entries: [
      { title: "Wave C gun identity SFX (Hushiron / Backtalk / Lamplighter / Faultlink)", body: "the eight silent `takes: []` hooks for Wave C guns now ship authored one-shots (`wave-c-guns-v1`) — planted hush-slug fire, Backtalk stub/parry/return (stub thin, return heavy), Lamplighter bead + warm light-patch plant, and Faultlink conductive fire + taut A↔B link latch. Selected takes land as `public/audio/sfx/{stem}_v1.{ogg,mp3}` (no `_vN` dump) and are bound in `waveSpec.ts`. Remastered for Vale EAR BLOCK: onset trim ~8ms, `loudnorm` + `alimiter=.89`, peaks < -1 dBFS; denser hushiron/faultlink fire sources; faultlink_link regenerated (rejected late/empty take). No Resonant Fork / Carry-the-Light bleed; generic `enemyHit` still covers body impact." },
