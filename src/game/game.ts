...
// Per-pet voice: a move cue (while trotting), a settle cue (on stop), an optional trot
// loop, and an optional happy cue (paired with the client-only attack emote on owner fire).
// The doggie has the richest set (a felt trot loop + pant); the others get a small
// species move/settle (+ happy for the Sable-PASS pack). Cooldowns live in the wave spec.
const PET_VOICES: Record<string, { move: string; settle: string; trot?: WaveEventId; happy?: WaveEventId }> = {
...
// Client-only attack-emote window (seconds). Plays the pet's attack strip while > 0; zero
// sim impact — purely a render juice cue on the owner's shot.
const PET_ATTACK_EMOTE_DUR = 0.42;
...
export class Game {
  ...
  private handleSimEvent(e: SimEvent) {
    switch (e.t) {
      case "shot": {
        ...
        // Client-only pet attack emote on owner fire (zero sim): flash the attack strip +
        // optional happy cue. Remotes are skipped above — only the local owner's pet reacts.
        this.triggerPetAttackEmote(LOCAL_ID);
  ...
  private renderPets() {
    ...
    for (const pet of this.petRenders.values()) {
      ...
      // Attack emote outranks walk/idle for its short window (client-only juice on fire).
  ...
  // Client-only: flash the equipped pet's attack strip (+ happy voice) when the owner fires.
  // Never touches the sim — pets remain pure cosmetics. Missing attack sheets fall back in
  // drawPetFrame (idle strip, then static base).
  private triggerPetAttackEmote(ownerId: string): void {
    const pet = this.petRenders.get(ownerId);
    if (!pet) return;
    pet.attackT = PET_ATTACK_EMOTE_DUR;
    if (ownerId !== LOCAL_ID) return;
