...
function waveAStateTests(): void {
  ...
  for (const value of [-1, 4, 1.5, null, "1", true] as WireValue[]) {
    const raw = JSON.parse(JSON.stringify(snapshotToWire(base))) as WireObject;
    raw.cat = value;
    let isRejected = false;
    try { validateSnap(raw); } catch { isRejected = true; }
...
function safetyTests(): void {
  ...
  check("protocol version is current (v47: contested hearth after the pet abilities roster)", PROTOCOL_VERSION === 47);
}
