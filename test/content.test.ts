...
function rotationTests(): void {
  ...
  }
  {
    ...
    // rotation. It is pinned for every seed; the seeded rotation resumes at F85 after Sever F55 /
    // Choirmaster F60 / Undertow F65 / Claimant F70 / Pale F75 / Wake F80 pins.
    let allGorge = true;
  ...
  {
    // F55 Sever + F60 Choirmaster + F65 Undertow + F70 Claimant + F80 Wake are FIXED set-pieces
    // (like F50 Gorge / F75 Pale). Seeded deep rotation resumes at F85. Never picks a pinned boss.
    let allSever = true;
    let allChoir = true;
    ...
    for (let s = 0; s < 80; s++) {
      ...
      if (bossKindForFloor(seed, 80) !== "wake") allWake = false;
    }
    ...
    check("F80 is ALWAYS Wake (fixed set-piece)", allWake);
    ...
    for (let s = 0; s < 80; s++) {
      const seed = 0x5EED + s * 977;
      let prev: EnemyKind | null = "undertow"; // F65 pin; F85 must not repeat it
      for (let floor = 70; floor <= 110; floor += 5) {
        if (floor === 70) { prev = "claimant"; continue; } // Claimant pin — don't bridge F65↔F85
        if (floor === 75) { prev = "pale"; continue; } // Pale pin — don't bridge F70↔F85
        ...
        if (a === "gorge" || a === "sever" || a === "choirmaster" || a === "undertow" || a === "claimant" || a === "wake" || a === "pale") neverPinned = false;
    ...
    check("seeded rotation never picks gorge/sever/choirmaster/undertow/claimant/wake/pale pins", neverPinned);
