...
section("tell watcher (frame observation)");
{
  ...
  check("submerge starts the underground track loop", eng.loopStarts.some((l) => l.key === "burrower.track#2"));
  dir.frame(frameInput([])); // burrower despawns while underground
  check("despawn sweeps the entity loop (no orphan)", eng.loopStops.includes("burrower.track#2"));
