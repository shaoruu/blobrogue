...
async function main(): Promise<void> {
  ...
  await test("P0-3: an input flood advances only ~1 fixed step per tick (no time advantage)", async () => {
    ...
    try {
      ...
      for (let i = 1; i <= 40; i++) ws.send(jsonCodec.encodeClient({ t: "input", seq: i, mx: 1, my: 0, aim: 0, fire: false, dash: false, act: false, ult: false, ackEv: 0, ackSnap: 0 }));
  ...
  await test("B6: a 240Hz client runs sustained without tripping any rate limit; an input flood dies", async () => {
    ...
    try {
      ...
      await waitUntil(() => (s.server.getWorld()?.playerCount ?? 0) >= 1, 2000);
      for (let i = 1; i <= 100; i++) flooder.send(jsonCodec.encodeClient({ t: "input", seq: i, mx: 0, my: 0, aim: 0, fire: false, dash: false, act: false, ult: false, ackEv: 0, ackSnap: 0 }));
