...
- `SHIPPED (private) / BLOCKED (public re-enable)` **PVP / ARENA (FFA deathmatch).** Private room-code ARENA is **LIVE**: the private rollout flag is enabled (#164) and the private room-code arena UI is unlocked (#177), while public PVP stays dark (fail-closed five-layer kill switch #127; `private_draft_v1` is the only signed policy; `ARENA · PATCHING` on any public entry is expected, not a bug). MVP #121 (frag-limit respawn, 100 HP, mode-gated pure sim, no fork), Patch 0 server hardening #128 (canDamage/respawn lifecycle, co-op byte-identical, PvP goldens + real-socket E2E), and client presentation #129 (HP bar not 100 hearts, match HUD scoreboard/timer/countdown/respawn/result, all co-op floor/GO-DOWN chrome suppressed; verified live in a real rendered arena) are all `SHIPPED`. **Fun pass** (game designer + balancer specced): Wave 1 = per-weapon knockback + lethal-pit ring-out env-kills + combo juice + mid-match blessing draft + comeback marker + instant re-queue; Wave 2 = contested center + hazards + kit counters; Wave 3 = PvP ult rework + objective mode. **Public re-enable gate = a fun/complete loop + a real live played match; PVP stays public-disabled until then, and automation must never flip `PVP_PUBLIC_ENABLED`.**
