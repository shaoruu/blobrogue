...
- `SHIPPED (private) / BLOCKED (public re-enable)` **PVP / ARENA (FFA deathmatch).** Private room-code ARENA is **LIVE**: the private rollout flag is enabled (#164) and the private room-code arena UI is unlocked (#177), while public PVP stays dark (fail-closed five-layer kill switch #127; `private_draft_v1` is the only signed policy; `ARENA · PATCHING` on any public entry is expected, not a bug). MVP #121 (frag-limit respawn, 100 HP, mode-gated pure sim, no fork), Patch 0 server hardening #128, client presentation #129, and Wave 1 fun pass #130 (per-weapon knockback + lethal-pit ring-out + combo juice + mid-match blessing draft + comeback + instant re-queue) are all `SHIPPED`. **Wave 2 SHIPPED private-only (PROTOCOL 47–48):** contested hearth + `ember_edge` (#199), ring-weather director tar/gust/spark (#200), draft counters brace/sight/rip (#201). Verified in real harness captures (`wave2-live-hearth` / contested / spark) + suites. Pets stay combat-OFF in arena (cosmetic auto-cast only). **Wave 3 PR1 SHIPPED private-only (PROTOCOL 49, #210):** arena-only kit ults (Gunner salvo / Mender triage / Bulwark shove / Phantom slip) on the frag DM — co-op ults hard-banned in arena, charge dumps to 0 on death, all damage through canDamagePlayer; VFX escalated (#218) + Quill FFA balance PASS (all in-band). Siege/objective mode still future. **Public re-enable gate = a real live played match on the Wave 2 loop; PVP stays public-disabled until then, and automation must never flip `PVP_PUBLIC_ENABLED`.**
- `SHIPPED` **Pet abilities (PROTOCOL 45–46, live).** Framework + Doggie FETCH / Wick PINPRICK (#196), remaining six verbs (#197), then autonomous smart-AI auto-cast with no V bind (#198). Server-authoritative, reconnect-safe, PVP hard-OFF for combat pets. Live on GS + Vercel.
...
- `SHIPPED` Boss roster (#31, weaver has a golden): Slime King (F5), Marrow blind charger (F15), Weaver (F20), Gilded Warden (F25), Hollow Choir (F30) implemented in `src/sim/enemies.ts`. `SHIPPED` Wave 1 deep bosses (#80, live, protocol v17): JET F35 (corrupted-mirror, 3 phases), THE TITHE F40 (feeder + 2-state destructible slab), QUORUM F45 (3 shared-HP husks + code-drawn tether + merge). GORGE F50 giant art staged (#79) as the F50 cap + giant-language reference (encounter wiring pending).
