...
incident rescue.** The normal path is the post–Stage-C operational deploy path. Two fixed admin
actions may mutate one named authoritative co-op room to recover a softlock or load a floor for
owner testing. They do not add a game-client command or change the game protocol.
...
fields are rejected. Warp-tainted runs emit no account-progression participants.
