...
Status: canonical. Scope: **production deployment/control plane plus narrowly gated live-run
incident rescue.** The normal path is the post–Stage-C operational deploy path. Four fixed admin
actions may mutate, snapshot, or restore one named authoritative co-op room to recover a softlock,
preserve a run across restart, or load a floor for owner testing. They do not add a game-client
command or change the game protocol.
...
| `GET /v1/worlds` | bounded world summaries (`id`, `players`, `tick`, `floor`, `names`, `away`) |
...
`script`, `shell`. Release routes accept only a `releaseId` (strict charset, must resolve in the
ReleaseStore). Rescue routes accept only a validated `worldId` and a floor in `1..1000`.
...
| `POST /v1/worlds/warp` | `{ worldId, floor }` | no | rebuild the named co-op room at a real generated floor, preserving all player loadouts |
| `POST /v1/worlds/force-open-exit` | `{ worldId }` | no | clear stuck encounter/combat/route blockers and open the named co-op room's exit |
| `POST /v1/worlds/snapshot` | `{ worldId }` | no | persist the room's build, floor, identities, and reconnect seats |
| `POST /v1/worlds/restore` | `{ worldId }` | no | rehydrate the room on a fresh floor generated by the current build |
...
Deploy-class mutates return an **operation** (`{ operationId, kind, state }`); progress is polled
via `GET /v1/operations/:id`. Rescue actions return their applied world/floor/player count
immediately and are append-only audited. Control signs the exact internal action with
`BRC_GS_CONTROL_SECRET`; gs verifies it with the matching `GS_CONTROL_SECRET`, a five-second TTL,
and single-use `jti`. Unsigned requests, game tickets, PVP worlds, missing worlds, and extra body
fields are rejected. Warp-tainted and restored runs emit no account-progression participants.

Run snapshots use build+floor fidelity. They persist the authoritative player projection, offer
history, run seed/floor/party lock, weapon bag, and verified reconnect seats to
`GS_RUN_SNAPSHOT_DIR` (default `/var/lib/blobrogue/run-snapshots`, mode 0600). Enemy positions and
partial encounter progress are intentionally not replayed. Restore generates floor N with the
deployed code, restores every co-op player's build and HP, and installs all seats atomically.
Snapshotted generations remain active through deploy FLUSH and are restored before WebSocket
admission opens.
...
| `BRC_GS_SYNTHETIC_TICKET_SECRET` | required in production; enables full synthetic join verify (loopback only) |
| `BRC_GS_CONTROL_SECRET` | dedicated signer for live-run rescue calls; matches gs `GS_CONTROL_SECRET` |
