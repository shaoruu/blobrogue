...
- TIMING / INPUT: hard grace is nested inside the total 3.0s shield; both endpoints share one authoritative spawn tick. Held offense during grace is latched and cannot auto-fire at 1.25s — all offense inputs must release, then re-press. Suppression emits one rate-limited arming pulse, never dry-fire/jam feedback.
...
- RESPAWN SELECTION: server-only scoring combines live-foe distance, pit clearance, actual wall/intact-cover LOS, authoritative aim, swept projectile/effect ETA (including homing acquisition), predicted damage, camping exclusion, and each player's last two spawn indices. Zero near-projectile threat and blocked LOS are lexicographic safety tiers before pit/distance. Ties use the lowest candidate index.
- RESPAWN_WAIT_SAFE: if all eight candidates are threatened when the ordinary timer ends, keep the player in RESPawning state and re-evaluate every 0.10s for at most 0.75s. Spawn as soon as one candidate becomes safe; at timeout choose minimum predicted 1.5s incoming damage and arm the ordinary full 3.0s shield.
