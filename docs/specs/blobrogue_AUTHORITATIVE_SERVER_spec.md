...
- **STAGE D — ONE PERSISTENT WORLD (collapse floors).** Server holds one persistent world instead of per-floor; remove run-end; death→hub respawn. (The open-world pivot, now on authoritative ground.)
- **STAGE E — INTEREST MGMT / REGIONS + chunk streaming.** Server shards the world by region; snapshots are per-interest. Scale.
- **STAGE F — WORLD BOSSES + drop-in public worlds + persistence wiring** (Convex saves gear/Amber/boss-flags; server loads on join, writes on events).
