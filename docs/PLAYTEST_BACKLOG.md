...
🟡 MORE HATS / COSMETICS (asked 3×) — WAVE 1: ALL 17 AD-APPROVED. Orientations deriving. WIRING ORDER TRAP (AD warning, CRITICAL): convex/cosmeticsCore.ts — ONLY face_round has an assetKey today; resolveOverlay() falls back to the procedural painter for ids without one. So: (1) add all 17 PNGs, (2) wire an assetKey on EVERY catalog entry + ADD catalog rows for the brand-new ids (wizard/beanie/helmet/chef/flower/mushroom/horns/headphones + eyepatch/star_shades/3d_glasses), (3) confirm each resolves to its PNG, ONLY THEN delete the procedural cosmeticArt.ts painters. Deleting painters first = un-wired cosmetics render INVISIBLE. Then render real socket composite (drawLoadoutOverlays) for AD placement gate. [art complete: 17 cosmetics oriented + 3 re-derives done; awaiting AD final confirm → then WIRE (assetKeys+rows first, then delete painters)]
...
🟢→🟡 Legendary gun TEXTURES — SAME as umbra fix: all 5 legendaries (umbra/reaper/hive/midas/lodestone) generated to AD briefs, oriented cosmetics done too. Gating → wire. [all 5 legendaries: winners picked, held derived; awaiting AD held confirm → drop files]
