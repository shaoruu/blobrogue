...
🟢 SHIPPED Name change — FIXED (verified in code 2026-07-12): signed-in accounts now keep a customName distinct from the Google name (players.ts ensureAccountRow: "the chosen customName wins; only fall back to the Google name when none is set"), and the account name field saves via setCustomName. WAS DIAGNOSED: GUEST rename path works + is test-covered. The failure = SIGNED-IN (Google) accounts: name field disabled (menu.ts:424) AND server overwrites with the Google account name (players.ts ensureAccountRow:181), so a signed-in rename reverts = 'doesn't work'. FIX: either allow an account displayName distinct from Google name (stop ensureAccountRow clobbering), or make the read-only UI copy explain why. Client+Convex only, no game-server deploy. [fix queued]
...
🟢 SHIPPED (#73, part of the heart fix) Consuming hearts when already at/over full — resolved with the Heart Container tithe-before-clamp fix.
...
🟢 SHIPPED (#107) BULLET ART — comprehensive pass done: every projectile weapon now has a drawBulletFx recipe (was falling through to a plain circle), incl. all 5 legendaries (reaper/hive/midas/umbra/lodestone, AD-gated shape-first) + lastlight/breach/frostline/sentry. bulletfx.test pixel-guards it. No plain-circle projectiles remain.

## NEW SYSTEMS / FEATURES
🟡 XP + KIT/CLASS system with ULTIMATES — DESIGN COMPLETE (game designer). v1 = 4 kits: GUNNER (Overdrive burst), MENDER/healer (Sanctuary heal zone), BULWARK/tank (Aegis barrier), PHANTOM/mobility (Phase). Universal ult meter (server-owned, charges by damage+kills+time floor, 8s min between casts, HUD-shown separate from Resonance). XP = ACCOUNT MASTERY track (not a currency — gates ACCESS to kits/cosmetics, keeps Coins/Amber as only spendables). Kit-select pre-run in Amber Camp lobby, per-player (co-op comps emerge, no forced roles). In-run stays blessings (NOT adding a parallel in-run level). v2 deferred: Broodmother/Emberwright kits, per-kit mastery, optional NT-style in-run-XP→blessing-pick (Ian's call later). Big — phase into content build waves. 🟢 SHIPPED LIVE — v1 kits+ult+account-Mastery XP (#83, protocol v18); kit SIGNATURES (#102, overheat/overshield/heal-pulse/dash-mark); ult LEGIBILITY (#101, charge motes + READY-by-name + kit badge); solo/co-op now APPLY the selected kit (#103) + the kit picker chip on PLAY SOLO. All 4 kits (Gunner/Mender/Bulwark/Phantom) live + gated.
🔵 SEE HEALTH — HP numbers + teammate HP: bundled as hard dependency of the kit build (bc-b7bc4443), building now.
...
## PVP MVP — post-ship playtest watch items
- 🟡 WATCH: late-match cover-degradation swinginess. As breakable arena cover thins toward end of match, sightlines open → raw-aim deletes. Balancer confirms this does NOT warrant a global TTK change (would tax the common full-cover early game to fix a rare late edge case; the 35% per-hit cap + flat 4.5s TTK already prevent instant deletes). If it shows in playtest, correct lever = a small late-match COVER REGEN or soft frag-race accelerator, NOT a TTK nerf. Only act if it actually shows.
