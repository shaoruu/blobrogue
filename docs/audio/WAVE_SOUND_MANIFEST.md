...
|The Deep — SUPERSEDED (audio director FINAL + selection): continuous bed is authored SILENCE (no `deep_loop`). Near-silent per-channel emitter: mineral tick every 2–4s @35% chance gain .08–.12; architecture shift every 5–9s @15% gain .10–.13; max ONE sounding; suppressed ±250ms around combat locks. Resin channels EMPTY until the 8 replacement analogues are selected — the selected channels never speed up to fill the gap|SELECTED takes only: `amb/deep_mineral_tick_v1/_v2`, `amb/deep_architecture_shift_v1`; resin selections pending|.08–.13|Mono, ≤1.2s, no melody, no reverb tail. Do NOT generate `deep_loop`; extend `SELECTED_DEEP_TAKES` (waveSpec.ts) when the replacements land.|
