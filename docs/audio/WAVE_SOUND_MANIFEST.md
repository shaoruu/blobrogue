...
|`burrower.track` SUPERSEDED (audio director FINAL): no continuous loop. Deterministic keyed positional emitter while burrowed — dirt grind every 1.0–1.4s @.22, pebble 0.35–0.75s @.14, shell scrape 1.3–2.0s @.18, underground thud only on direction-lock @.28|`enemy/burrow_dirt_vN`, `enemy/burrow_pebble_vN`, `enemy/burrow_scrape_vN`, `enemy/burrow_thud`|≤0.5 each|see left|—|3/3/2/1|Component prompts: dry mono transient-forward digging grind / tiny stone chatter / chitin shell drag / muffled deep body knock; no tails, they overlap by design. Do NOT generate `burrow_track`.|
...
|The Deep — SUPERSEDED (audio director FINAL): continuous bed is authored SILENCE (no `deep_loop`). Sparse positional emitter instead: one authored event every 1.5–3.5s from resin creak 35% / mineral tick 35% / architecture shift 15% / resin drip 15%; max 2 overlapping; gains .08–.16; suppressed ±250ms around combat locks|`amb/deep_resin_creak_vN` (2), `amb/deep_mineral_tick_vN` (3), `amb/deep_arch_shift_vN` (2), `amb/deep_resin_drip_vN` (2)|.10–.16|Component prompts: faint jet-black resin creak under tension / tiny cold mineral tick / broken architecture settling at a wrong angle / one thick resin drip; mono, ≤1.2s, no melody, no reverb tail. Do NOT generate `deep_loop`.|
